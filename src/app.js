...
app.post("/subscribers/EnterData", async (req, res) => {
  ...
  try {
    // const response = await subscribersModel.create(data );
    const newUser = await new subscribersModel(data);
...
app.put("/subscribers/UpdateData/:id", async (req, res) => {
  ...
  try {
    // const response = await subscribersModel.create(data );
    const updateRes = await subscribersModel.findByIdAndUpdate(
      {
        _id: id,
      },
      inputData
...
app.delete("/subscribers/DeleteData", async (req, res) => {
  ...
  try {
    // const response = await subscribersModel.create(data );
    const DeleteRes = await subscribersModel.deleteOne({
      _id: id,
    });
...
app.get("/subscribers", async (req, res) => {
  let data = await subscribersModel.find();
  res.send(data);
...
app.get("/subscribers/:id", cors(), async (req, res, next) => {
  try {
    const result = await subscribersModel.find({ _id: req.params.id });
